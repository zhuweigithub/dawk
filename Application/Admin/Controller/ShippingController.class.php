...
//dump($list);
	...
	{
		//sub_name: sub_name, sub_id: sub_id, staple_arr: staple_arr
		$sub_name   = $_POST['sub_name'];
		$sub_id     = $_POST['sub_id'];
		$staple_arr = $_POST['staple_arr'];
		$staple_arr = json_decode($staple_arr, true);
		$result     = M("Staple_rule")->where("store_id=" . $sub_id)->find();
		$arr        = array(
			"store_id"     => $sub_id,
			"store_name"   => $sub_name,
			"first_weight" => $staple_arr[0]['first_weight_a'],
		);
		$id         = 0;
		if (count($result) > 0) {
			$arr['id'] = $result['id'];
		...
		if ($id > 0) {
			$db   = M('Staple_rule_ext');
			for ($i = 0; $i < count($staple_arr); $i++) {
				$arr1 = array(
					"staple_id"             => $id,
					"zone_id"               => $staple_arr[$i]['zone_id'],
					"zone_name"             => $staple_arr[$i]['zone_name'],
					"first_weight_a"        => $staple_arr[$i]['first_weight_a'],
					"first_fee_a"           => $staple_arr[$i]['first_fee_a'],
					"first_weight_b"        => $staple_arr[$i]['first_weight_b'],
					"first_fee_b"           => $staple_arr[$i]['first_fee_b'],
					"second_weight_start_a" => $staple_arr[$i]['second_weight_start_a'],
					"second_weight_end"     => $staple_arr[$i]['second_weight_end'],
					"second_fee_a"          => $staple_arr[$i]['second_fee_a'],
					"second_weight_start_b" => $staple_arr[$i]['second_weight_start_b'],
					"second_fee_b"          => $staple_arr[$i]['second_fee_b'],
				);
				$param['staple_id'] = $id;
				$param['zone_id'] = $staple_arr[$i]['zone_id'];

				$staple_rule = $db->where($param)->find();
				if(count($staple_rule) > 0){
					$arr1['id'] = $staple_rule['id'];
					$db->save($arr1);
				}else{
					$db->add($arr1);
				}
